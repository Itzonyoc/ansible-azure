            ...
            steps {
                script {
                    withCredentials([azureServicePrincipal('sp-azure-curso-devops')]) {
                        ...
                        sh "ansible-playbook main.yml -i hosts -e subscription_id=${AZURE_SUBSCRIPTION_ID} -e client_id=${AZURE_CLIENT_ID} -e secret_id=${AZURE_CLIENTE_SECRET} -e tenant_id=${AZURE_TENANT_ID} -v"
                    }
                }
            }
        }
